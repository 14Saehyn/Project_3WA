...
const PrivateRoute = ({children, auth = null}) => {
    ...
    **/
    ...
    const {admin, user, id} = state.user;
    useEffect(() => {
    ...
    useEffect(() => { if (id || !auth) setLoading(false) },[user, location])
    ...
    */
    const isUserAuthorized = isPublic || (isLimitedToAdmin && admin) || (isLimitedToConnected && user);
